...
public class KdTree {
    ...
    private static class Node {
        ...
        public int compareTo(RectHV rect) {
            // Need to cache the values for x and y because the Coursera autograder sets
            // limits on the number of calls to point.x() and point.y()
            // This is likely due to the assumption that we will use RectHV and
            // its intesect() method in the code, something I've elected not to do.
            double x = point.x();
            double y = point.y();

            if (level % 2 == 0) {
                // When rect.xmax() == point.x(), search both subtrees as points
                // lying on the rectangle border might exist in the right subtree
    ...
    private Node insertAt(Node node, Point2D p, int level, double xmin, double xmax,
                          double ymin, double ymax, Node parent) {
        if (node == null) {
        ...
        // We keep track of the grandparent node to determine where the splitting
        // line for the new child node should stop
        if (node.compareTo(p) <= 0) {
            if ((level + 1) % 2 == 0) {
                node.right = insertAt(node.right, p, node.level + 1,
                                      p.x(),
                                      p.x(),
                                      node.point.y(), node.parent.ymax, node);
            }
            else {
                // parent == null when 1st horizontal line is drawn
                node.right = insertAt(node.right, p, node.level + 1,
                                      node.point.x(),
                                      node.parent != null ? node.parent.xmax : 1.0,
                                      p.y(), p.y(), node);
            }
        }
        else {
            if ((level + 1) % 2 == 0) {
                node.left = insertAt(node.left, p, node.level + 1,
                                     p.x(), p.x(), node.parent.ymin, node.point.y(), node);
            }
            else {
                node.left = insertAt(node.left, p, node.level + 1,
                                     node.parent != null ? node.parent.xmin : 0.0,
                                     node.point.x(), p.y(), p.y(), node);
            }
        }
        return node;
    }
    ...
    public Iterable<Point2D> range(RectHV rect) {
        ...
        // Initially used an ArrayList for storing the points,
        // but it appears to have too much overhead
    ...
    // Originally compared shortest distance from point to entire axis instead
    // of just the splitting line segment in nearest()
    // This results in extra exploration of subtrees which should have been pruned ->
    // use x/y min/max variables in Node and compare to them instead
    private Point2D nearest(Node node, Point2D p, Point2D nearestPoint) {
        ...
        // Putting this at the end rather than at the beginning will lead to extra
        // nodes being explored
        // since the entire height has to be explored before the current node is considered
        if (node.distSquaredToLine(p) < nearestPoint.distanceSquaredTo(p) &&
                node.point.distanceSquaredTo(p) < nearestPoint.distanceSquaredTo(p)) {
            nearestPoint = node.point;
        }

        // When coordinates are equal, both subtrees are checked regardless
        // of the first result since distSquaredToLine(p) = 0
        if (node.compareTo(p) <= 0) {
